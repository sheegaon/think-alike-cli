...
import os, json, requests, socketio
...
def load_cfg():
    ...
    if os.path.exists(p):
        with open(p,"r") as f: user = json.load(f)
...
class REST:
    ...
    def call(self, name: str, *, path=None, params=None, body=None, headers=None):
        ...
        headers = headers or {}
        print("\n[REST ->] %s %s" % (m, url))
        if params: print("        params=%s" % json.dumps(params))
        if body is not None: print("        body=%s" % json.dumps(body))
    ...
    while True:
        ...
        try:
            if cmd in ("help","?"):
                ...
                print("Type 'jc' to quick-join casual, 'jo' competitive, 'l' list rooms, 'cr <stake> <tier>' to create.")
            ...
            # Player
                ...
                if not args:
                    print("Usage: pu <username>"); continue
                uname = args[0]
                ...
                print(json.dumps({"player_id": sess.player_id, "username": sess.username, "room_key": sess.room_key}, indent=2))
            ...
            # Rooms
                ...
                if not args: print("Usage: jr <room_key>"); continue
                rk = args[0]
                data = rest.call("rooms_join", body={"room_key": rk, "as_spectator": False, "player_id": sess.player_id})
                if isinstance(data, dict):
                    sess.room_key = rk
                    sess.room_token = data.get("room_token") or data.get("token")
                    print("[STATE] joined room_key=%s room_token=%s" % (sess.room_key, sess.room_token))
            elif cmd == "cr":
                if len(args)<2: print("Usage: cr <stake> <tier>"); continue
                stake=float(args[0]); tier=args[1]
                data = rest.call("rooms_create", body={"stake": stake, "tier": tier})
                if isinstance(data, dict):
                    sess.room_key = data.get("room_key") or data.get("id")
                    print("[STATE] created room_key=%s" % (sess.room_key))
            elif cmd == "obs":
                if not args: print("Usage: obs <room_key>"); continue
                rk=args[0]
                data = rest.call("rooms_join", body={"room_key": rk, "as_spectator": True, "player_id": sess.player_id})
                if isinstance(data, dict):
                    sess.room_key = rk
                    sess.room_token = data.get("room_token") or data.get("token")
                    print("[STATE] observing room_key=%s" % (sess.room_key))
            elif cmd == "lr":
                data = rest.call("rooms_leave", body={"room_key": sess.room_key, "player_id": sess.player_id})
                print("[STATE] left (if server accepted).")
                sess.room_key = None; sess.room_token=None
            elif cmd == "skip":
            ...
            # WS connect
                ...
                if not args: print("Usage: ws on|off"); continue
                sub=args[0]
                if sub=="on":
                    if ws is None: ws = WS(cfg["WS_URL"], cfg.get("WS_NAMESPACE",""))
                    ws.connect()
                elif sub=="off":
                    if ws: ws.disconnect()
                else:
            ...
            # WS helpers
                ...
                if len(args)<2: print("Usage: wse <event> <json>"); continue
                ev=args[0]; payload=" ".join(args[1:])
                try: data=json.loads(payload)
                except: data=payload
                if ws: ws.emit(ev, data)
                else: print("[WARN] WS not connected")

            elif cmd == "commit":
                if not args: print("Usage: commit <idx>"); continue
                if not ws: print("[WARN] WS not connected"); continue
                ...
                if not ws: print("[WARN] WS not connected"); continue
                if not args: print("Usage: queue on|off"); continue
                want = args[0].lower()=="on"
                ws.emit("spectator_queue", {"want_to_join": want})
            elif cmd == "emote":
                if not ws: print("[WARN] WS not connected"); continue
                if not args: print("Usage: emote <emoji>"); continue
